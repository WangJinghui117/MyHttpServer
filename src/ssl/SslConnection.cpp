    ...
    , messageCallback_(nullptr)
{
    ...
    // 设置连接回调
    conn_->setMessageCallback(bind(&SslConnection::onRead, this, placeholders::_1, placeholders::_2, placeholders::_3));
}
...
void SslConnection::send(const void* data, size_t len){
    if(state_ != SSLState::ESTABLISHED){
        LOG_ERROR << "Cannot sent data before SSL handshake is completed";
        return;
    }
    ...
    while((pending = BIO_pending(writeBio_)) > 0){
        int bytes = BIO_read(writeBio_, buf, min(pending, static_cast<int>(sizeof(buf))));
        if(bytes > 0){
...
int SslConnection::bioRead(BIO* bio, char* data, int len){
    ...
    size_t toRead = min(static_cast<size_t>(len), readable);
